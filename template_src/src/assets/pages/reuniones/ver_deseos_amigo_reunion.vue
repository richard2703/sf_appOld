                            ...
                            <div class="row w-100 m-0 pl-3">
                                <div class="row w-100 m-0 py-1" v-for="p of a.productos" :key="p.id">
                                    ...
                                    <div class="col my-auto px-0 pl-3">
                                        ...
                                        <div class="row w-100 m-0 letra-gray4-18">{{p.producto.total | currency}} {{p.producto.currency}}</div>
...
const moment = require('moment')
    export default{
        ...
        computed: {
            ...
            f(){return this.invitaciones.find(x=>{return x.id == this.detalle }) || {} },
            evento(){return this.f.evento || {} },
            misListas(){ return [this.f.lista || {}] },
            usuarios(){return this.$store.getters.userStateArray('usuarios');},
        },
        mounted() {
            this.$store.dispatch('synchronizeData');
            console.log("_-_-__------", this.detalle, this.invitaciones, this.f, this.misListas);
        },
        methods:{
            nuevo(id){
            ...
            add(e){
                ...
                console.log("AGREGANDO USUARIO", this.usuarios.find(w=>w.id == this.evento.usuarios_id) );
                this.$store.commit('setCarritosState',['externo', 1]);
                this.$store.commit('setCarritosState',['para_usuarios_id', this.evento.usuarios_id]);
                this.$store.commit('setCarritosState',['usuario', this.usuarios.find(w=>w.id == this.evento.usuarios_id) ]);
