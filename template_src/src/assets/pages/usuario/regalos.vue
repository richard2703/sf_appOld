...
import _ from 'lodash';
    ...
    export default{
        ...
        computed: {
            ...
            paginado(){
                if(this.busqueda){
                    ...
                    // return JSON.parse(JSON.stringify( this.productos.filter(x=>{return this.filtro(x) }) ))
                    this.productos.map(a=>a.order = this.filtro(a) );
                    return (_.orderBy( this.productos.filter( x =>  x.order ) , ['order'], ['desc'] )).filter((x,y)=>y <= this.cantidad);
        ...
        methods:{
            ...
            noModal(){
                this.$store.dispatch('postGetSyncForm', [ {modal1: 1} ]);
            },
            ...
            filtro( x, cx = false){
                ...
                if(x.descripcion.toLowerCase().includes(this.busqueda.toLowerCase()) ){
                    // console.log("DESCRIPTION RES", this.busqueda, x.descripcion);
                    return 9.5;
                }
                if(x.partner && x.partner.id){
                    if(x.id == '1016'){
                        // console.log("=> => =>", (x.partner || {}).tradename, ((x.partner || {}).tradename || '').toLowerCase().includes(this.busqueda.toLowerCase()) );
                    }
                    if(((x.partner || {}).tradename || '').toLowerCase().includes(this.busqueda.toLowerCase()) ){
                        // console.log("RETUNING", x);
                        return 9;
                    }  
                }
                ...
                if(nn.some( w => bus.some( b => w.includes(b) ) )){
                    let add = 0;
                    bus.map((r, y)=>{
                        if( ['1016', '470'].some(w=>w==x.id) ){
                            // console.log("=>", r, nn, nn.some(w=>w.includes(r)));
                        }

                        if(nn.some((w)=>w.includes(r))){
                            add += 1-((y+1)/10);
                        }
                    });
                    return 8 + (add ? add/10 : 0);
                }
